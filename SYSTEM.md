...
- **Privacy & Consent** section: privacy notice (`PrivacyNoticeModal`), "Your consent" summary → `ConsentModal` to change who consented / SMS alerts, **Download a copy of my data** (share sheet, JSON), **Erase my LifeTap tag** (`WriteNFC` with `mode: 'erase'`), **Withdraw consent & delete my data** (`eraseEverything()` — cloud account via `delete-account` if signed in + local profile — then offers to erase the tag, passing the deleted profile's id as `ownId`; **Delete Account** does the same), privacy contact (`CONTROLLER.contact`). Tag status here and on the Profile screen uses `isTagCurrent()` — the same check as Home — so a plaintext or rotated-key tag never shows as "Synced". About shows `package.json`'s `version`
...
- **Notice text:** `src/legal/privacyNotice.ts` — `PRIVACY_NOTICE_VERSION`, `NOTICE_SUMMARY` (first layer), `NOTICE_SECTIONS` (full notice), `RESPONDER_UNDERTAKING`, and `CONTROLLER` (ArchTech is the controller for the capstone pilot and handles all privacy concerns at `CONTROLLER.contact`, archtechbtg@gmail.com — no DPO or university escalation until an LGU is involved; pilot data is deleted within 30 days of the defense). Bump the version on material changes → every user is sent through `ConsentGate` again.
